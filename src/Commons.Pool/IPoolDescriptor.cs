...
namespace Commons.Pool
{
	...
	public interface IPoolDescriptor<T> where T : class
	{
		...
		/// access every time from the pool manager by providing its key. While if an object 
		...
		/// instantiated, the pool descriptor sets it to default value 10 andchecks whether its larger than <see cref="InitialSize"/>.
